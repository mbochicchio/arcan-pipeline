...
fetch_analyses_page <- function(conn, offset = 0, page_size = 5) {
    q_page <- "
        SELECT a.id as analysis_id, a.date_analysis,
            a.file_result as file_result_id,
            p.id as project_id, p.name, p.language,
    ...
    graphs %>%
        ...
        left_join(page, by = c("graph_id" = "file_result_id")) %>%
...
new_data_generator <-
    generator(function(conn, max_analyses = -1, page_size = 5, offset = 0) {
        i <- offset

        repeat {
            cat(paste("Saving project", i))
            page_data <-
                fetch_analyses_page(conn, offset = i, page_size = page_size)
            n_page <- nrow(page_data)
            if (n_page <= 0 || (max_analyses > 0 && i > max_analyses)) {
                break
            }
            i <- i + n_page
            ...
            for (index in seq_len(n_page)) {
                ...
                df_nodes$project_id <- analysis$project_id
                df_nodes$project_analysis_id <- analysis$analysis_id
                df_nodes$project_analysis_date <- as.character(analysis$date_analysis)
                df_nodes$project_name <- analysis$name
                df_nodes$project_language <- analysis$language
                df_nodes$project_version <- analysis$version
                df_nodes$project_version_date <- as.character(analysis$version_date)

                df_edges$project_id <- analysis$project_id
                df_edges$project_analysis_id <- analysis$analysis_id
                df_edges$project_analysis_date <- as.character(analysis$date_analysis)
                df_edges$project_name <- analysis$name
                df_edges$project_language <- analysis$language
                df_edges$project_version <- analysis$version
                df_edges$project_version_date <- as.character(analysis$version_date)
                df_edges$index <- NULL
...
output_file <- args[1]
max_analyses <- as.numeric(args[2])
conn <- get_connection_from_env()
tryCatch(
    {
        fetch_data <- new_data_generator(conn, max_analyses = max_analyses)
